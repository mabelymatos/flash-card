criaCartao(
    'O Que Faz Esse Profissional?',
    'O que uma nail designer realmente faz?',
    'Uma nail designer é uma artista que cria e aplica designs estéticos e artísticos nas unhas, usando diversas técnicas e materiais para deixá-las bonitas e personalizadas.'
)

criaCartao(
    'Qual a Durabilidade?',
    'Quanto tempo dura um alongamento de unhas feito por uma boa nail designer?',
    'Geralmente, um alongamento bem feito pode durar de duas a três semanas antes de precisar de manutenção, dependendo do crescimento da sua unha e dos seus cuidados.'
)

criaCartao(
    'Materiais Utilizados',
    'Quais os principais materiais que uma nail designer usa?',
    'Utilizamos uma variedade de produtos, como gel, fibra de vidro, esmaltes em gel, pós cromados, pedrarias e adesivos para criar os diferentes estilos.'
)

criaCartao(
    'Como Escolher o Melhor?',
    'O que devo procurar ao escolher um nail designer? ',
    'Procure por uma profissional com bom portfólio, que siga boas práticas de higiene, use materiais de qualidade e tenha boas recomendações de outros clientes.'
)